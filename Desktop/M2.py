...
while True:
    ...
    # Move head left
    if inp == Commands.HEADLFT:
        curChannel = 3
        curPosition = x.getPosition(curChannel)
        if curPosition < max_range:
            curPosition += 1000
    # Move head right
    elif inp == Commands.HEADRT:
        curChannel = 3
        curPosition = x.getPosition(curChannel)
        if curPosition > min_range:
            curPosition -= 1000
    # Move head up
    elif inp == Commands.HEADUP:
        curChannel = 4
        curPosition = x.getPosition(curChannel)
        if curPosition < max_range:
            curPosition += 1000
    # Move head down
    elif inp == Commands.HEADDWN:
        curChannel = 4
        curPosition = x.getPosition(curChannel)
        if curPosition > min_range:
            curPosition -= 1000
    # Move body left
    elif inp == Commands.BODYLFT:
        curChannel = 0
        curPosition = x.getPosition(curChannel)
        if curPosition < max_range:
            curPosition += 1850
    # Move body right
    elif inp == Commands.BODYRT:
        curChannel = 0
        curPosition = x.getPosition(curChannel)
        if curPosition > min_range:
            curPosition -= 1850
    elif inp == Commands.FORWARD:
        curChannel = 2
        curPosition = x.getPosition(curChannel)
        speeds = [ 3750, 4250, 5000, 5750, 5000, 4250, 3750]
        for speed in speeds:
            x.setTarget(curChannel, speed)
            sleep(.23)
    elif inp == Commands.BACKWARD:
        curChannel = 2
        curPosition = x.getPosition(curChannel)
        speeds = [3750, 3500, 2750, 1800, 2750, 3500, 3750]
        for speed in speeds:
            x.setTarget(curChannel, speed)
            sleep(.23)
    # Reset head and body servos to center
    # Sets mototrs to neutral
    elif inp == Commands.Zero:
        x.setTarget(0, 6000)
        x.setTarget(1, 3750)
        x.setTarget(2, 3750)
        x.setTarget(3, 6000)
    ...
    if (inp != 0 & inp != 8):
        x.setTarget(curChannel, curPosition)
